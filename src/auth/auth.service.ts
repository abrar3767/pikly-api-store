...
export class AuthService {
  ...
  private sign(user: any) {
    ...
    return { token: this.jwtService.sign(payload), expiresIn: '7d' }
  }
  ...
  async login(dto: LoginDto) {
    ...
    if (!user.isActive) throw new UnauthorizedException({ code: 'ACCOUNT_BANNED', message: 'Your account has been suspended' })

    await this.userModel.findByIdAndUpdate(user._id, { lastLogin: new Date().toISOString() })
  ...
  async refreshToken(dto: RefreshTokenDto) {
    try {
      ...
      // Reject tokens older than 30 days even with ignoreExpiration
      const issuedAt  = payload.iat * 1000
      const thirtyDays = 30 * 24 * 60 * 60 * 1000
      if (Date.now() - issuedAt > thirtyDays) throw new Error('Token too old')
