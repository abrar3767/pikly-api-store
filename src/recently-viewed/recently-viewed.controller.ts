...
export class RecentlyViewedController {
  ...
  @Get()
  @ApiOperation({ summary: 'Get recently viewed products — supports offset (page) and cursor pagination' })
  @ApiQuery({ name: 'page',   required: false, description: 'Page number — use either page OR cursor, not both' })
  @ApiQuery({ name: 'limit',  required: false, description: 'Items per page (default: 10)' })
  @ApiQuery({ name: 'cursor', required: false, description: 'Cursor from previous response for cursor pagination' })
  async getRecent(
    @Request() req: any,
