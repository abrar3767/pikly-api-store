...
describe('CartService', () => {
  ...
  describe('applyCoupon', () => {
    ...
    it('throws COUPON_NOT_APPLICABLE when no cart items match restrictions', async () => {
      ...
      cartModel.findOne.mockResolvedValue(mockCartDoc({ items: [{ productId: 'prod1', subtotal: 100 }] }))
    ...
    it('throws COUPON_ALREADY_USED when authenticated user has already redeemed the coupon (BUG-03)', async () => {
      couponModel.findOne.mockResolvedValue({
        code: 'USED10', type: 'percentage', value: 10, isActive: true,
        expiresAt:    new Date(Date.now() + 86_400_000),
        usedCount:    1, usageLimit: 100, minOrderAmount: 0,
      ...
      await expect(service.applyCoupon({ code: 'USED10', sessionId: 'sess1' }, 'uid-already-used'))
        .rejects.toThrow(BadRequestException)
    })
  })
  ...
  describe('mergeCart', () => {
    ...
    it('drops out-of-stock items from guest cart during merge (BUG-07)', async () => {
      const guestDoc = mockCartDoc({
        sessionId: 'guest1',
        items: [{ productId: 'p-oos', quantity: 2, subtotal: 100, price: 50, originalPrice: 50, variantId: null, stock: 5 }],
        coupon: null,
      })
      const userDoc = mockCartDoc({ sessionId: 'user1', items: [], coupon: null })

      cartModel.findOne
        .mockResolvedValueOnce(guestDoc)
