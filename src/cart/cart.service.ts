...
export class CartService {
  ...
  private computeSummary(cart: CartDocument) {
    ...
    return {
      ...
      pricing: {
        subtotal, shipping,
        shippingNote: shipping === 0 ? 'Free shipping applied' : `Add $${(50 - subtotal).toFixed(2)} more for free shipping`,
        tax, taxRate: '10%', discount, total,
  ...
  async addItem(dto: AddToCartDto) {
    ...
    if (existing) {
      ...
      if (newQty > stock) throw new BadRequestException({ code: 'INSUFFICIENT_STOCK', message: `Only ${stock} units available` })
      existing.quantity = newQty
      existing.subtotal = parseFloat((price * newQty).toFixed(2))
    } else {
      if (dto.quantity > stock) throw new BadRequestException({ code: 'INSUFFICIENT_STOCK', message: `Only ${stock} units available` })
      items.push({
  ...
  async applyCoupon(dto: ApplyCouponDto) {
    ...
    if (coupon.usedCount >= coupon.usageLimit) throw new BadRequestException({ code: 'COUPON_LIMIT_REACHED', message: 'Coupon usage limit has been reached' })
