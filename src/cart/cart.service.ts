...
export class CartService {
  ...
  async addItem(dto: AddToCartDto) {
    ...
    cart.items = items
    await cart.save()
    this.logger.log(`Cart addItem: session=${dto.sessionId} product=${dto.productId} qty=${dto.quantity}`)
    return this.computeSummary(cart)
  }
  ...
  async applyCoupon(dto: ApplyCouponDto, userId: string | null = null) {
    ...
    await cart.save()
    this.logger.log(`Coupon applied: session=${dto.sessionId} code=${coupon.code} userId=${userId ?? 'guest'}`)
    return this.computeSummary(cart)
  }
  ...
  async mergeCart(dto: MergeCartDto) {
    ...
    for (const gItem of (guest.items ?? []) as any[]) {
      ...
      const liveStock = liveProduct
        ? (gItem.variantId
            ? (liveProduct.variants?.find((v: any) => v.variantId === gItem.variantId)?.stock ?? liveProduct.inventory.stock)
            : liveProduct.inventory.stock)
      ...
      if (existing) {
        ...
        userItems.push({ ...gItem, quantity: safeQty, stock: liveStock, subtotal: parseFloat((gItem.price * safeQty).toFixed(2)) })
      }
    }
