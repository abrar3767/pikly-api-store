...
export class CompareService {
  ...
  compare(productIds: string[]) {
    if (productIds.length < 2) throw new BadRequestException({ code: 'MIN_PRODUCTS', message: 'At least 2 products required' })
    if (productIds.length > 4) throw new BadRequestException({ code: 'MAX_PRODUCTS', message: 'Maximum 4 products can be compared' })
    ...
    for (const key of allAttrKeys) {
      attributeComparison[key] = products.map(p => ({ productId: p.id, value: p.attributes?.[key] ?? '—' }))
    }
    ...
    const cards = products.map(p => ({ id:p.id,slug:p.slug,title:p.title,brand:p.brand,media:p.media,pricing:p.pricing,ratings:p.ratings,inventory:{stock:p.inventory.stock},shipping:p.shipping,attributes:p.attributes,tags:p.tags }))
