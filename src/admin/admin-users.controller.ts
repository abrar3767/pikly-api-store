...
export class AdminUsersController {
  ...
  @ApiQuery({ name:'search', required:false }) @ApiQuery({ name:'role', required:false }) @ApiQuery({ name:'isActive', required:false })
  async findAll(@Query('page') page?: number, @Query('limit') limit?: number, @Query('search') search?: string, @Query('role') role?: string, @Query('isActive') isActive?: string) {
    const filter: any = {}
    ...
    return successResponse({ users, pagination: { total, page:p, limit:l, totalPages:Math.ceil(total/l), hasNextPage:p*l<total } })
  }
  ...
  async ban(@Param('id') id: string) {
    const user = await this.userModel.findByIdAndUpdate(id, { isActive:false }, { new:true }).lean()
    if (!user) return successResponse(null, { message: 'User not found' })
    const { passwordHash, ...safe } = user as any
  ...
  async unban(@Param('id') id: string) {
    const user = await this.userModel.findByIdAndUpdate(id, { isActive:true }, { new:true }).lean()
    if (!user) return successResponse(null, { message: 'User not found' })
    const { passwordHash, ...safe } = user as any
  ...
  async changeRole(@Param('id') id: string, @Body() body: { role: string }) {
    if (!['customer','admin'].includes(body.role)) return successResponse(null, { message: 'Role must be customer or admin' })
    const user = await this.userModel.findByIdAndUpdate(id, { role:body.role }, { new:true }).lean()
    if (!user) return successResponse(null, { message: 'User not found' })
    const { passwordHash, ...safe } = user as any
    return successResponse(safe)
  }
