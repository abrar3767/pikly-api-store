...
export class AdminBannersController {
  ...
  async create(@Body() body: any) {
    return successResponse(await this.homepageService.adminCreateBanner({
      id:`ban_${Date.now()}`, title:body.title, subtitle:body.subtitle??'',
      image:body.image??null, ctaText:body.ctaText??'', ctaLink:body.ctaLink??'',
      position:body.position??'hero',
      startDate:body.startDate??new Date().toISOString(),
      endDate:body.endDate??new Date(Date.now()+365*86_400_000).toISOString(),
      isActive:body.isActive??true, sortOrder:body.sortOrder??99,
    }))
  }
  ...
  async toggle(@Param('id') id: string) {
    ...
    if (!banner) return successResponse(null, { message: `Banner "${id}" not found` })
