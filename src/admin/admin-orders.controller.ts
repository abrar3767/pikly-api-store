...
export class AdminOrdersController {
  ...
  async stats() {
    const result = await this.orderModel.aggregate([{ $group: { _id: '$status', count: { $sum: 1 } } }])
  ...
  @ApiQuery({ name:'status', required:false }) @ApiQuery({ name:'userId', required:false }) @ApiQuery({ name:'search', required:false })
  async findAll(@Query('page') page?: number, @Query('limit') limit?: number, @Query('status') status?: string, @Query('userId') userId?: string, @Query('search') search?: string) {
    const filter: any = {}
    ...
    return successResponse({ orders, pagination: { total, page:p, limit:l, totalPages:Math.ceil(total/l), hasNextPage:p*l<total } })
  }
  ...
  async findOne(@Param('orderId') orderId: string) {
    ...
    if (!order) return successResponse(null, { message: `Order ${orderId} not found` })
  ...
  @ApiParam({ name:'orderId' })
  async updateStatus(@Param('orderId') orderId: string, @Body() body: { status: string; message?: string }) {
    const valid = ['pending','confirmed','processing','shipped','delivered','cancelled']
    if (!valid.includes(body.status)) return successResponse(null, { message: `Invalid status. Must be one of: ${valid.join(', ')}` })
    ...
    order.timeline.push({ status: body.status, timestamp: now, message: body.message ?? `Status updated to ${body.status} by admin` })
    await order.save()
    return successResponse(order)
  }
  ...
  @ApiParam({ name:'orderId' })
  async addTracking(@Param('orderId') orderId: string, @Body() body: { trackingNumber: string; estimatedDelivery?: string }) {
    const order = await this.orderModel.findOne({ orderId })
    if (!order) return successResponse(null, { message: `Order ${orderId} not found` })
    const now = new Date().toISOString()
