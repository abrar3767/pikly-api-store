...
function filterWithout(products: any[], query: Record<string,any>, excludeKey: string): any[] {
  ...
  if (excludeKey !== 'category'    && query.category)    result = result.filter(p => p.category    === query.category)
  if (excludeKey !== 'subcategory' && query.subcategory) result = result.filter(p => p.subcategory === query.subcategory)
  ...
  if (excludeKey !== 'minPrice' && query.minPrice != null) result = result.filter(p => p.pricing.current >= Number(query.minPrice))
  if (excludeKey !== 'maxPrice' && query.maxPrice != null) result = result.filter(p => p.pricing.current <= Number(query.maxPrice))
  if (excludeKey !== 'rating'   && query.rating)           result = result.filter(p => p.ratings.average >= Number(query.rating))

  const boolFlags = ['featured','bestSeller','newArrival','trending','topRated','onSale','inStock','freeShipping']
  for (const flag of boolFlags) {
    if (excludeKey !== flag) {
      const val = parseBool(query[flag])
      if (val !== null) {
        if (flag === 'inStock')      result = val ? result.filter(p => p.inventory.stock > 0) : result.filter(p => p.inventory.stock === 0)
        else if (flag === 'freeShipping') result = result.filter(p => p.shipping?.freeShipping === val)
  ...
  if (excludeKey !== 'attrs' && query.attrs) {
    ...
    for (const pair of pairs) {
      ...
      if (key && value) result = result.filter(p => p.attributes && String(p.attributes[key]).toLowerCase() === value.toLowerCase())
    }
  }

  if (query.q && query.q.trim()) {
    const fuse = new Fuse(result, { keys:['title','brand','description','tags'], threshold:0.3 })
...
export function buildFacets(allFilteredProducts: any[], activeFilters: Record<string, any>) {
  ...
  for (const [key, vals] of Object.entries(attrMap)) {
    attributes[key] = Object.entries(vals)
      .sort((a,b) => b[1]-a[1])
      .map(([value, count]) => ({ value, count, selected: activeAttrs[key] === value.toLowerCase() }))
  }
  ...
  return {
    ...
    ratings, attributes, badges, shipping,
  }
}
