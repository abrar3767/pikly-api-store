...
export interface FilterQuery {
  ...
  inStock?: boolean;  freeShipping?: boolean; featured?: boolean
  bestSeller?: boolean; newArrival?: boolean; trending?: boolean
  topRated?: boolean;   onSale?: boolean
  attrs?: string;    sort?: string
...
export function filterProducts(products: any[], query: FilterQuery) {
  ...
  if (query.minPrice != null) result = result.filter(p => p.pricing.current >= Number(query.minPrice))
  if (query.maxPrice != null) result = result.filter(p => p.pricing.current <= Number(query.maxPrice))
  if (query.rating)           result = result.filter(p => p.ratings.average >= Number(query.rating))

  const boolFlags = ['featured','bestSeller','newArrival','trending','topRated','onSale'] as const
  ...
  if (inStock !== null) result = inStock
    ? result.filter(p => p.inventory.stock > 0)
    : result.filter(p => p.inventory.stock === 0)

  const freeShipping = parseBool(query.freeShipping)
  if (freeShipping !== null) result = result.filter(p => p.shipping?.freeShipping === freeShipping)
  ...
  switch (query.sort ?? 'relevance') {
    case 'price_asc':     result.sort((a,b) => a.pricing.current - b.pricing.current); break
    case 'price_desc':    result.sort((a,b) => b.pricing.current - a.pricing.current); break
    case 'rating_desc':   result.sort((a,b) => b.ratings.average - a.ratings.average); break
    case 'newest':        result.sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()); break
    case 'bestselling':   result.sort((a,b) => (b.inventory?.sold??0) - (a.inventory?.sold??0)); break
    case 'discount_desc': result.sort((a,b) => b.pricing.discountPercent - a.pricing.discountPercent); break
    default: if (query.q) result.sort((a:any,b:any) => (a._score??1)-(b._score??1)); break
  }

  // ── 4. Paginate ──────────────────────────────────────────────────────────
  const paginated = smartPaginate(result, { page: query.page, limit: query.limit??20, cursor: query.cursor })
  ...
  if (query.brand)      query.brand.split(',').forEach(b => appliedFilters.push({ key:'brand', value:b.trim(), label:`Brand: ${b.trim()}` }))
  ...
  if (parseBool(query.inStock)===true)      appliedFilters.push({ key:'inStock',      value:true, label:'In Stock' })
  if (parseBool(query.freeShipping)===true) appliedFilters.push({ key:'freeShipping', value:true, label:'Free Shipping' })
  for (const f of boolFlags) {
    if (parseBool((query as any)[f])===true) appliedFilters.push({ key:f, value:true, label:f })
  }
  if (query.attrs) {
